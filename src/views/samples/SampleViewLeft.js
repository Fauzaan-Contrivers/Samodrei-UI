                ...
                <Grid container>
                  <Grid item md={5} xs={12}>
                    <Box sx={{ display: 'flex', marginBottom: 2.7 }}>
                      ...
                      <Typography variant='body2'>{data?.Quantity_20}</Typography>
                    </Box>
                    <Box sx={{ display: 'flex', marginBottom: 2.7 }}>
                      <Typography sx={{ marginRight: 2, fontWeight: 500, fontSize: '0.875rem' }}>
                        Quantity 60 MG:
                      </Typography>
                      <Typography variant='body2'>{data?.Quantity_60}</Typography>
                    </Box>
                    <Box sx={{ display: 'flex', marginBottom: 2.7 }}>
                      <Typography sx={{ marginRight: 2, fontWeight: 500, fontSize: '0.875rem' }}>Status:</Typography>
                      <Typography variant='body2'>{String(data?.Status).toUpperCase()}</Typography>
                    </Box>
                  </Grid>
                  <Grid item md={7} xs={12}>
                    <Box sx={{ display: 'flex', marginBottom: 2.7 }}>
                      ...
                      <Typography variant='body2'>{moment(data?.Pre_Sign_Date).format('YYYY-MM-DD')}</Typography>
                    </Box>
                    <Box sx={{ display: 'flex', marginBottom: 2.7 }}>
                      <Typography sx={{ marginRight: 2, fontWeight: 500, fontSize: '0.875rem' }}>
                        Deliver Date:
                      ...
                      <Typography variant='body2'>
                        {String(data?.Post_Sign_Date) != 'null'
                ...
                <Grid container justifyContent={'space-between'}>
                  <Grid item sx={5} style={{ display: 'flex', flexDirection: 'column' }}>
                    <img
                      src={
                        data.Pre_Sign
                          ? `data:image/png;base64,${data.Pre_Sign}`
                        ...
                        if (data.Pre_Sign_Location__Latitude__s && data.Pre_Sign_Location__Longitude__s) {
                          window.open(
                            'https://www.google.com/maps?saddr=' +
                              data.Pre_Sign_Location__Latitude__s +
                              ',' +
                  ...
                  <Grid item sx={5} style={{ display: 'flex', flexDirection: 'column' }}>
                    <img
                      src={
                        data.Post_Sign
                          ? `data:image/png;base64,${data.Post_Sign}`
                          : 'https://st4.depositphotos.com/14953852/22772/v/600/depositphotos_227725020-stock-illustration-image-available-icon-flat-vector.jpg'
                      }
                      style={{ width: 120, height: 120, fontWeight: 600, marginBottom: 4, fontSize: '3rem' }}
                    />
                    ...
                    <a
                      ...
                      onClick={() => {
                        if (data.Post_Sign_Location__Latitude__s && data.Post_Sign_Location__Longitude__s) {
                          window.open(
                            'https://www.google.com/maps?saddr=' +
                              data.Post_Sign_Location__Latitude__s +
                              ',' +
                  ...
                  <Grid item sx={5} style={{ display: 'flex', flexDirection: 'column' }}>
                    <Box sx={{ display: 'flex', marginBottom: 2.7 }}>
                      ...
                      <Typography variant='body2'>{moment(data?.Post_Sign_Date).format('YYYY-MM-DD')}</Typography>
                    </Box>
                  </Grid>
                </Grid>
              </Box>
