      ...
      >
        ...
        <DialogActions className="dialog-actions-dense">
          ...
          <Button
            variant="contained"
            onClick={() =>
              onDownloadClickHandler(start, end, productAdvocate, sampleStatus)
            }
          >
            Download
          </Button>
...
const SampleTableHeader = (props) => {
  ...
  const downloadCSV = (start, end, productAdvocate, sampleStatus) => {
    ...
    samples.forEach(function (row) {
      ...
      csv += `${row.Quantity_20},`;
      csv += `${row.Quantity_60},`;
      csv += `${row.Prescriber_Name},`;
      csv += `${row.Prescriber_Street},`;
      csv += `${row.Prescriber_City},`;
      csv += `${row.Prescriber_State},`;
      csv += `${row.Prescriber_Zip},`;
      csv += `${row.Prescriber_Npi},`;
      csv += `${preSignDate},`;
      csv += `${postSignDate},`;
      csv += `${String(row.Status).toUpperCase()},`;
      csv += "\n";
    });
