...
const TableHeader = props => {
  ...
  const downloadCSV = (start, end) => {
    ...
    if (start) {
      jobsData = jobsData.filter(val => moment(val.question_l_0).isSameOrAfter(moment(start), 'D'))
    }
    if (end) {
      jobsData = jobsData.filter(val => moment(val.question_l_0).isSameOrBefore(moment(end), 'D'))
    }
    ...
    lunchData.forEach(function (row) {
      csv += `${row.Prescriber__r.First_Name},`
      csv += `${row.Prescriber__r.Last_Name},`
      csv += `${row.Prescriber__r.Speciality},`
      csv += `${row.Prescriber__r.NPI},`
      csv += ',' // unit
      csv += `"${row.Prescriber__r.Street_Address?.replace(/#/g, '')}",`
      csv += `${row.Prescriber__r.City},`
      csv += `${row.Prescriber__r.State},`
      csv += `${row.Prescriber__r.Zip},`
      csv += `${row.Prescriber__r.Phone},`
