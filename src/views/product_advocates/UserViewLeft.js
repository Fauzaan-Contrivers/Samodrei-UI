...
const UserViewLeft = ({ data, jobsData, samplesData }) => {
  ...
  useEffect(() => {
    let flagCountSubmitted = jobsData.filter(
      val => val.Product_Advocate == data.SalesforceId && val.Status == 'Feedback completed'
    )
  ...
  useEffect(() => {
    if (data.SalesforceId) {
      const temp = jobsData.filter(val => val.Product_Advocate == data.SalesforceId && Boolean(val.question_1))
