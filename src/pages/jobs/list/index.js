...
const InvoiceList = () => {
  ...
  const onFilterChangeHandler = () => {
    ...
    if (productAdvocateValue) {
      data = data.filter(val => val.product_advocate.SalesforceId == productAdvocateValue)
    }

    if (startDateRange) {
      data = data.filter(val => moment(val.feedback_submitted_at).isSameOrAfter(moment(startDateRange), 'D'))
    }
    if (endDateRange) {
      data = data.filter(val => moment(val.feedback_submitted_at).isSameOrBefore(moment(endDateRange), 'D'))
    }
    ...
    if (selectedPrescriber) {
      data = data.filter(val => val.prescriber.SalesforceId == selectedPrescriber)
    }

    if (revisits) {
      data = [...new Map(data.map(item => [item.prescriber.SalesforceId, item])).values()]
    }
  ...
  const handleRadiusValue = e => {
    dispatch(onJobFilterChangeHandler({ filter: 'difference_location_doctor', value: e.target.value }))
  }
  ...
  const handleWhoDidYouMeetWith = event => {
    ...
    dispatch(onJobFilterChangeHandler({ filter: 'meet_with', value: selectedValues }))
  }
  ...
  const cancelJobData = async row => {
    // Update the API
    console.log(row.SalesforceId)
    dispatch(cancelJob({ job_id: row.SalesforceId }))
    // Update the state
  ...
  const jobsListViewColumns = [
    ...
    },
    {
      ...
      field: 'Status',
      headerName: 'Status',
      renderCell: ({ row }) => <Typography variant='caption'>{row?.Status}</Typography>
    },
    {
      flex: 0.5,
        ...
        return (
          <Box sx={{ display: 'flex', alignItems: 'center' }}>
            <Box sx={{ display: 'flex', flexDirection: 'column' }}>
              <Link href={`/prescribers/preview/${prescriber.SalesforceId}`} passHref>
    ...
    },
    {
      ...
      renderCell: ({ row }) => {
        ...
        return (
          <Box sx={{ display: 'flex', alignItems: 'center' }}>
            <Box sx={{ display: 'flex', flexDirection: 'column' }}>
              <Link href={`/product_advocates/preview/${product_advocate.SalesforceId}`} passHref>
    ...
    },
    {
      ...
      renderCell: ({ row }) => (
        <Typography variant='body2' style={{ color: row.selected_far_doctor ? 'red' : 'green' }}>
          {/* {parseFloat(row.Distance_To_Doctor).toFixed(2)} */}
          {row?.Distance_To_Doctor === null ? ' ' : parseFloat(row?.Distance_To_Doctor).toFixed(2)}
        </Typography>
      )
    },
    {
      ...
      renderCell: ({ row }) => (
        <Typography variant='body2'>
          {row?.Time_Spent_At_Job === null ? ' ' : parseFloat(row?.Time_Spent_At_Job).toFixed(2)}
        </Typography>
      )
    },
    {
