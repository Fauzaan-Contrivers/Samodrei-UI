...
const InvoiceList = () => {
  ...
  const handleStatusValue = e => {
    dispatch(onProductAdvocateStatusChangeHandler({ filter: 'Active', value: e.target.value }))
  }
  ...
  const handleSubView = (event, userId) => {
    ...
    dispatch(onProductAdvocateStatusChangeHandler({ filter: 'sub_view', value: selectedValues }))
  }

  const handlePreview = (e, userId) => {
    handlePreviewUpdateHandler(userId, e.target.value)
    dispatch(onProductAdvocateStatusChangeHandler({ filter: 'preview', value: e.target.value }))
  }
  ...
  const defaultColumns = [
    ...
    },
    {
      ...
      renderCell: ({ row }) => <Typography variant='body2'>{row?.Stock_20 || '0'}</Typography>
    },
    {
      flex: 0.2,
      minWidth: 110,
      ...
      renderCell: ({ row }) => <Typography variant='body2'>{row?.Stock_60 || '0'}</Typography>
    },
    {
      flex: 0.2,
      minWidth: 90,
      ...
      renderCell: ({ row }) => (
        ...
        >
          {row?.Active == '1' ? <Check sx={{ fontSize: '1rem' }} /> : <Close sx={{ fontSize: '1rem' }} />}
        </CustomAvatar>
      )
    },
    {
      ...
      renderCell: ({ row }) => (
        <CustomAvatar
          ...
          sx={{ width: '1.875rem', height: '1.875rem' }}
        >
          {row?.Admin ? <Check sx={{ fontSize: '1rem' }} /> : <Close sx={{ fontSize: '1rem' }} />}
        </CustomAvatar>
      )
    },
    {
    ...
    },
    {
      ...
      headerName: 'Sub View',
      renderCell: ({ row }) => {
        const subView = row?.SubView?.split(';').map(subView => subView + ' ')
        return (
          <Grid item xs={12} sm={12}>
            <FormControl fullWidth variant='standard' sx={{ border: 'none' }}>
              <Select
                ...
                labelId='invoice-status-select'
                value={
                  store.product_advocates.filter.sub_view || row?.SubView?.split(';')?.map(subView => subView + ' ')
                }
                renderValue={() => subView}
                multiple
              >
